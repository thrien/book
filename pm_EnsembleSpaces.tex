...
\begin{defn}
<<<<<<< HEAD
	Let $\Ens$ be a quantifiable ensemble space. Let $\{X_i\}_{i = 1}^n$ be a set of statistical quantities, not necessarily complete. The \textbf{coarse graining} of $\Ens$ over $\{X_i\}_{i = 1}^n$ is the manifold $\mathcal{M} \subseteq \Ens$ represented by the ensemble that maximizes the entropy for each fixed value of the quantities. That is, there exists a chart $\phi : \mathbb{R}^n \to \mathcal{M}$ such that $X^i(\psi(x^i)) = x^i$ and $S(\psi(x^i)) \geq \ens$ for all $\ens \in \Ens$ such that $X^i(\ens) = x^i$.
=======
...
>>>>>>> 16f95cbed5ff58488d92f6e2bc492fecc5af6a0d
\end{defn}
